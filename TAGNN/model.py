...
class SessionGraph(Module):
    def __init__(self, opt, n_node):
        ...
        no_decay = ["bias", "batch_norm", "activation"]
        ...
        self.optimizer = torch.optim.Adam(optimizer_grouped_parameters, lr=opt.lr, weight_decay=opt.l2)
        # self.optimizer = torch.optim.Adam(self.parameters(), lr=opt.lr, weight_decay=opt.l2)
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, step_size=opt.lr_dc_step, gamma=opt.lr_dc)
        self.gradient_accumulation=opt.gradient_accumulation
...
def train_test(model, train_data, test_data):
    ...
    # for i, j in tqdm(zip(slices, np.arange(len(slices))),total=len(slices),leave=True,position=0):
        model.optimizer.zero_grad()
        targets, scores = forward(model, i, train_data)
        targets = trans_to_cuda(torch.Tensor(targets).long())
        loss = model.loss_function(scores, targets - 1)
        loss.backward()
        if model.gradient_accumulation:
            if (j+1)%model.accumulation_steps == 0 or j==len(slices)-1:
