...
parser.add_argument('--dataset_path', default='amex_explorepoi', help='dataset name:diginetica/yoochoose/sample/amex_log-poi/amex_log-category/amex_log-context/amex_explorepoi-poi_category/amex_explorepoi-poi_context/amex_explorepoi-category/amex_explorepoi-context/')
...
if args.dataset_path in amex_sets:
    dataset_loc = '/home/ubuntu/michelle_ML/s3_file_processing/dataset_files/amex-yoochoose-train-test-files/'
    args.dataset_path = dataset_loc + args.dataset_path + '/'
else: 
    args.dataset_path = 'datasets/' + args.dataset_path + '/'
...
def main():
    ...
    if args.dataset_path.split('/')[-2] == 'diginetica':
        ...
        n_items = 37484
    elif args.dataset_path.split('/')[-2] in ['amex_explorepoi-poi_category', 'amex_explorepoi-poi_context']:
        n_items = 551 #line 182 print(item_ctr) in preprocess.py
    elif args.dataset_path.split('/')[-2] in ['amex_explorepoi-category', 'amex_category']:
        n_items = 4 
        args.topk = 1
    elif args.dataset_path.split('/')[-2] in ['amex_explorepoi-context']:
        n_items = 183 
    elif args.dataset_path.split('/')[-2] in ['amex_log-poi']: #amex_poi
        n_items = 551 #line 182 print(item_ctr) in preprocess.py
    elif args.dataset_path.split('/')[-2] in ['amex_log-context']: #amex_context
        n_items = 183 
    else:
